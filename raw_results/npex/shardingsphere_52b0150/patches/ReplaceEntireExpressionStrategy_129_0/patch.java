...
private static java.util.List<org.apache.shardingsphere.infra.executor.sql.context.PrimaryKeyMetaData> getPrimaryKeyColumns(final org.apache.shardingsphere.infra.metadata.ShardingSphereMetaData metaData, final java.util.Collection<org.apache.shardingsphere.infra.route.context.RouteMapper> tableMappers) {
    ...
    for (org.apache.shardingsphere.infra.route.context.RouteMapper each : tableMappers) {
        /* NPEX_PATCH_BEGINS */
        result.add(metaData.getRuleSchemaMetaData().getSchemaMetaData().get(each.getLogicName()) != null ? new org.apache.shardingsphere.infra.executor.sql.context.PrimaryKeyMetaData(each.getLogicName(), metaData.getRuleSchemaMetaData().getSchemaMetaData().get(each.getLogicName()).getPrimaryKeyColumns()) : new org.apache.shardingsphere.infra.executor.sql.context.PrimaryKeyMetaData());
    }
    return result;
}
}
